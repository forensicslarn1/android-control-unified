...
function ProfilesWorkspace({ isLive, output, refresh }: { isLive: boolean; output: string; refresh: () => void }) {
  return <section className="space-y-5"><div className="service-card p-6"><p className="kicker text-[#687584]">Android Enterprise context</p><h2 className="mt-2 text-3xl font-bold tracking-[-0.05em]">Separate profiles, not a universal clone switch.</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-[#526273]">Android Work Profile separates managed work apps and data from personal data. ADB cannot safely create or clone every app into a profile on every phone; this inspector reveals the current user/profile topology before offering any device-specific next step.</p><div className="mt-6 flex flex-wrap gap-3"><Button disabled={!isLive} onClick={refresh} className="action-button bg-[#14253a] text-[#f6f2ea] hover:bg-[#223952]"><UsersRound className="mr-2" size={16} />Inspect users & profiles</Button><a href="https://www.android.com/enterprise/work-profile/" target="_blank" className="action-button inline-flex items-center border border-[#14253a] px-4 py-2 text-sm font-medium">Work Profile guide <ArrowRight className="ml-2" size={15} /></a></div></div><div className="grid gap-5 lg:grid-cols-[.85fr_1.15fr]"><div className="border border-[#d8d1c4] bg-[#fff2e1] p-5"><p className="kicker text-[#934639]">Guardrail</p><p className="mt-3 text-sm leading-6 text-[#6d3d35]">An install-to-profile operation is only appropriate after this device confirms an eligible profile and the required policy authority. The desk does not pretend that “clone app” is a standard, portable ADB command.</p></div><div className="service-card p-5"><div className="flex items-center justify-between"><div><p className="kicker text-[#687584]">Current result</p><h3 className="mt-1 font-bold">`pm list users`</h3></div><ListFilter size={18} className="text-[#59869c]" /></div><pre className="mono mt-5 max-h-72 overflow-auto whitespace-pre-wrap border-l border-[#c8f04a] bg-[#f3efe6] p-4 text-xs leading-6 text-[#263d55]">{isLive ? output : "Connect a device to inspect users and managed profiles."}</pre></div></div></section>;
}

function ApkWorkspace({ isLive, install }: { isLive: boolean; install: (file?: File) => Promise<void> }) {
  return <section className="space-y-5"><div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]"><div className="service-card p-6"><p className="kicker text-[#687584]">APK desk</p><h2 className="mt-2 text-3xl font-bold tracking-[-0.05em]">Pick an APK. See the real install command.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-[#526273]">The selected APK is transferred over the active USB ADB session to a temporary device path, then installed using `pm install -r -g`. Nothing is uploaded to this application.</p><label className={`action-button mt-6 inline-flex items-center border px-4 py-3 text-sm font-semibold ${isLive ? "border-[#14253a] bg-[#14253a] text-[#f6f2ea]" : "cursor-not-allowed border-[#d8d1c4] bg-[#eee9df] text-[#687584]"}`}><Upload className="mr-2" size={16} />Select APK<input disabled={!isLive} type="file" accept=".apk,application/vnd.android.package-archive" className="sr-only" onChange={(event) => install(event.target.files?.[0])} /></label><p className="mono mt-5 text-[0.68rem] text-[#687584]">transfer → /data/local/tmp/&lt;sanitized-name&gt; · install → pm install -r -g &lt;path&gt;</p></div><div className="overflow-hidden border border-[#d8d1c4] bg-[#fffdf8]"><img src="/manus-storage/privacy-workstation_68e7bcbd.jpg" alt="Android phone ready for application management" className="h-48 w-full object-cover object-[55%_45%]" /><div className="p-5"><p className="kicker text-[#687584]">Installer guardrails</p><p className="mt-2 text-sm leading-6 text-[#526273]">The device remains the final authority. Signature mismatches, blocked installs, and policy errors are retained verbatim in the ledger.</p></div></div></div><div className="border-l-2 border-[#d39152] bg-[#fff2e1] p-4 text-sm leading-6 text-[#6d5133]"><strong>Use trusted APK sources.</strong> This desk transfers the file you choose; it does not verify the publisher, inspect a certificate, or bypass Android installation protections.</div></section>;
}

function FilesWorkspace({ isLive, path, setPath, files, loading, load }: { isLive: boolean; path: string; setPath: (value: string) => void; files: DeviceFile[]; loading: boolean; load: () => Promise<void> }) {
  return <section className="space-y-5"><div className="service-card p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="kicker text-[#687584]">File workbench</p><h2 className="mt-2 text-2xl font-bold tracking-[-0.04em]">Browse a device path with ADB Sync.</h2></div><div className="flex w-full gap-2 sm:max-w-lg"><input disabled={!isLive} value={path} onChange={(event) => setPath(event.target.value)} onKeyDown={(event) => event.key === "Enter" && load()} className="h-10 min-w-0 flex-1 border border-[#d8d1c4] bg-[#fffdf8] px-3 mono text-xs outline-none focus:border-[#14253a]" /><Button disabled={!isLive || loading} onClick={load} className="action-button bg-[#14253a] text-[#f6f2ea] hover:bg-[#223952]">{loading ? <Loader2 className="animate-spin" size={16} /> : <RefreshCw size={16} />}</Button></div></div><p className="mt-4 text-xs leading-5 text-[#687584]">Basic actions use the ADB file-sync channel. Advanced destructive file operations should be entered in Operator detail so their exact command is preserved.</p></div><div className="service-card overflow-hidden"><div className="flex items-center justify-between border-b border-[#d8d1c4] bg-[#f3efe6] px-4 py-3"><p className="mono text-xs text-[#526273]">{path}</p><span className="text-xs text-[#687584]">{files.length} entries</span></div><div className="min-h-64">{!isLive ? <div className="grid min-h-64 place-items-center text-sm text-[#687584]">Authorize a device to browse files.</div> : files.length === 0 ? <div className="grid min-h-64 place-items-center text-sm text-[#687584]">Select refresh to read this path.</div> : files.map((file) => <div className="flex items-center gap-3 border-b border-[#eee8dc] px-4 py-3" key={file.name}>{file.isDirectory ? <Folder size={17} className="text-[#59869c]" /> : <FileText size={17} className="text-[#687584]" />}<div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{file.name}</p><p className="mono text-[0.63rem] text-[#687584]">{file.isDirectory ? "directory" : `${file.size.toLocaleString()} bytes`}</p></div><ChevronRight size={16} className="text-[#a6b3be]" /></div>)}</div></div></section>;
}
